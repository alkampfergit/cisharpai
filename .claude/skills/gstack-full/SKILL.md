...
- `dotnet` toolchain available for the .NET 8 + 10 test runs
...
- **Never** auto-merge PRs from this loop. Stop at "PR opened, ready for review".
...
- Respect project-level rules — for this repo see `AGENTS.md` and `.claude/CLAUDE.md` (tests must be green; never read `.env`; update `memories/project_overview.md` on structural changes).
