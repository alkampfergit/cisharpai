...
- **Testing Hosted Retrieval**: `FakeHostedRetrievalFeature` in `Cisharpai.Testing` — per-store `FakeRetriever` dictionary. `AddStore(storeId)` registers a store, `ForStore(storeId)` returns its `FakeRetriever`, `GetRetriever(storeId)` accesses the fake for setup (enqueue responses, check captured queries). Unregistered store IDs throw `KeyNotFoundException`.
