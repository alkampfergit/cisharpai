...
- **Retrieval**: `IRetriever.RetrieveAsync(string query, RetrievalOptions options, CancellationToken)` returns `Task<IReadOnlyList<ScoredChunk>>`. Backend-agnostic — no vector vocabulary in the contract. The query is a `string`, not a vector. Common options are portable: `TopK`, `MinScore` (applies to returned `ScoredChunk.Score`), and `MetadataEquals` (AND-combined equality filters). `ProviderQuery` carries optional provider-specific extensions (`IRetrievalQueryExtension`); unsupported extension types must fail clearly (no silent ignore). `InMemoryRetriever(IEmbeddingClient, model?)` is a brute-force cosine-similarity demo/testing aid — load via `Add(TextChunk, float[])` / `AddRange(...)`, not for production. Returns empty on embedding failure instead of throwing.
