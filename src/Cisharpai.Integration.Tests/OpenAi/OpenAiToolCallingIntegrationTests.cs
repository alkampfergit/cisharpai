...
public sealed class OpenAiToolCallingIntegrationTests
{
    ...
    // --- ToolChoice.None Produces Text Response ---

    [TestCase("gpt-4.1-nano")]
    public async Task ToolCalling_ToolChoiceNone_ReturnsTextResponse(string model)
    {
        var client = CreateClient();
        var toolFeature = client.Features.Get<IToolCallingFeature>()!;
        ...
        Assert.That(response.IsSuccess, Is.True, $"Request failed: {response.ErrorMessage}");
        Assert.That(response.ToolCalls, Is.Null, "ToolChoice.None should not produce tool calls");
