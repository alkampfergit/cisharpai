...
- Azure OpenAI chat/json/tool-calling requests now recover from Azure routing mismatches by retrying the alternate endpoint or chat-completions token shape when the first guess is rejected, which fixes opaque or misleading deployment names without requiring every deployment to follow provider naming conventions.
