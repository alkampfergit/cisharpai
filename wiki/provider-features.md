...
| Grounded Chat (RAG) | `IGroundedChatFeature`; GPT-5 deployments only (Responses API); documents sent as `input_file` items; uses `ExecuteWithRouteFallbackAsync` — returns `IsSuccess=false` if deployment falls back to Chat Completions |
