...
Size and overlap count **Unicode scalar values**, not UTF-16 code units, grapheme clusters, bytes, or tokens. A supplementary character such as an emoji counts as one scalar; combining sequences can span chunks. `StartOffset` is a zero-based UTF-16 offset suitable for `document.Text.Substring(chunk.StartOffset, chunk.Text.Length)`. `EndOffset` is the exclusive UTF-16 end offset; for verbatim chunks it equals `StartOffset + Text.Length`. Both offsets are stored so that non-verbatim chunkers (e.g. contextual retrieval that prefixes generated text) can preserve the original source span for citations. `Metadata` is an `IReadOnlyDictionary<string, object?>` carrying chunker-specific data (e.g. boundary type, similarity score); it defaults to empty and is never null. Valid surrogate pairs are never split. Text and whitespace are preserved; empty documents yield no chunks, and no redundant overlap-only final chunk is produced. IDs must be nonblank, text must be nonnull, and callers own ID uniqueness. Chunk indices start at zero for each document.
