...
- **Azure OpenAI routing hints**: `AzureOpenAiClientOptions.ModelName` lets you specify the same underlying OpenAI model name you would use with the OpenAI client for an opaque deployment, and the chat client still retries the alternate endpoint or token-shape once when Azure rejects the first guess with a route-shape error. Learned mismatches are cached in-process per `(Endpoint, DeploymentName, ApiVersion)` so later dynamically created clients reuse them.
