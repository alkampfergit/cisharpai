...
- `OpenAiFileSearchRetriever` — Internal `IRetriever` that queries an OpenAI hosted vector store via the Responses API `file_search` tool. Maps `OpenAiFileSearchResult` → `ScoredChunk` with passage-relative `TextChunk` offsets. Honors common `RetrievalOptions` (`TopK` sent natively; `MinScore`/`MetadataEquals` applied post-search), rejects unsupported provider-query extensions, returns empty list on API errors (logs warning), and propagates config errors (`DefaultModel` missing) + cancellation.
