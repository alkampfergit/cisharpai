...
- `IRetriever` — backend-agnostic retrieval contract: `RetrieveAsync(string query, RetrievalOptions options)` returns ranked `ScoredChunk` results. Portable options are `TopK`, `MinScore`, and `MetadataEquals` (AND-equality filters), plus optional provider-specific `ProviderQuery` (`IRetrievalQueryExtension`). Unsupported provider-query types must fail clearly instead of being ignored.
- `InMemoryRetriever` — brute-force cosine-similarity demo/testing retriever. Takes `IEmbeddingClient` + pre-loaded `(TextChunk, float[])` pairs; applies `MetadataEquals` + `MinScore` + `TopK` in retrieval and rejects provider-specific query extensions. Not for production.
...
- `QueryTransformation/` — `IQueryTransformer` interface (`TransformAsync(string query) → IReadOnlyList<string>`), four LLM-based implementations: `QueryRewriter` (clean up filler, clarify intent; returns 1 query), `MultiQueryExpander` (generate N variants for independent retrieval + RRF fusion), `HydeTransformer` (Hypothetical Document Embeddings — embed a generated answer instead of the raw question), `StepBackTransformer` (broader abstract question for supporting context). All take `IChatCompletionClient` + optional `QueryTransformerOptions` (model, temperature) + optional custom system prompt. `CompositeQueryTransformer` chains transformers sequentially with deduplication.
- `Pipeline/` — `IRagPipeline` / `ConversationalRagPipeline` — composable orchestrator wiring query transform → retrieve → RRF fusion → rerank → context pack → grounded chat. `RagPipelineBuilder` fluently configures each stage; every stage is optional (degenerate pipelines valid). `ConversationQueryRewriter` wraps `IChatCompletionClient` to rewrite follow-up queries into standalone search queries given conversation history. `RagResult` carries answer, citations, retrieved/packed/dropped sets, and rewritten query for debugging. `RagStreamingChunk` for streaming. `RagPipelineOptions` per-call knobs (conversation history, topK, optional `Retrieval` options passthrough, reranker topN, packing, system prompt, model, temperature, citation mode). DI: `AddCisharpaiRagPipeline()`. Grounded chat via `IGroundedChatFeature` when available; fallback via `GroundedChatFallbackHelper`.
- `Evaluation/` — RAG quality measurement. `IRagEvaluator` interface for swappable LLM-as-judge scorers. Four implementations: `GroundednessEvaluator`, `AnswerRelevanceEvaluator`, `ContextPrecisionEvaluator`, `ContextRecallEvaluator` — each takes `IChatCompletionClient` with `IJsonOutputFeature`, returns `EvaluationScore(Score [0,1], Rationale)`. `RankingMetrics` static class: `Ndcg`, `Mrr`, `RecallAtK` — pure-function ranking metrics (no model calls).
- No storage drivers (Qdrant, pgvector, Pinecone, Redis). Existing embedding fakes, `FakeRetriever`, `FakeRagPipeline`, and `FakeQueryTransformer` support offline tests without core interface changes.
- Consumer guide: `wiki/rag.md`; unit tests: `src/Cisharpai.Tests/Rag/` — example-based per chunker, plus `ChunkerInvariantPropertyTests` running every chunker over a deterministic seeded corpus (ASCII, emoji, CJK, mixed scripts, whitespace-heavy, whitespace-free, surrogate-only, empty) across a sweep of size/overlap settings. Query transformation tests in `src/Cisharpai.Tests/Rag/QueryTransformation/`. Pipeline tests in `src/Cisharpai.Tests/Rag/Pipeline/`. Evaluation tests in `src/Cisharpai.Tests/Rag/Evaluation/`.
...
- `OpenAiFileSearchRetriever` — Internal `IRetriever` that queries an OpenAI hosted vector store via the Responses API `file_search` tool. Maps `OpenAiFileSearchResult` → `ScoredChunk` with passage-relative `TextChunk` offsets. Honors common `RetrievalOptions` (`TopK` sent natively; `MinScore`/`MetadataEquals` applied post-search), rejects unsupported provider-query extensions, and treats final `TopK` as best-effort when post-filters remove items from the provider window. Returns empty list on API errors (logs warning), and propagates config errors (`DefaultModel` missing) + cancellation.
...
- `FakeRetriever` — Fake for `IRetriever`. Response queue, default, query capture `(Query, RetrievalOptions)`, `Reset()`. No feature flags.
...
- `FakeRagPipeline` — Fake for `IRagPipeline`. Response queue, default, query capture for both `AskAsync` and `AskStreamingAsync`, `Reset()`.
- `FakeQueryTransformer` — Fake for `IQueryTransformer`. Response queue, default, query capture, `Reset()`.
- `FakeResponses` — Static factories: `Chat`, `ChatError`, `ToolCall`, `ToolCalls`, `GroundedChat`, `StreamingChunks`, `Embedding`, `Rerank`, `RerankError`, `TokenCounter`, `Retriever`, `RagAnswer`, `RagError`, `RagStreamingChunks`, `RagPipeline`, `QueryTransformer`, etc.
